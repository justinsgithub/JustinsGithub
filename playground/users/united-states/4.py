...
with open('us_places_all.txt') as file:
    for place in file.readlines():
        place = place.strip()
        page = 'p/united-states/' + place + '/kinksters'
        get_page(page)
        file_to_save = place + '/page-1' + '-kinksters.txt'
        kinkster_list = get_page_content('//main//div//a[@class="link f5 fw7 secondary mr1"]')
        save_data_list(kinkster_list, file_to_save)
