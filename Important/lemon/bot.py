...
def scrape_user_page(state):
    ...
    for x in range(len(user_num_pics)):
        ...
        user["state"] = state
        insert_data(jusers, user, False)
        insert_data(lusers, user, False)
        print(user)
...
def love_pictures():
    #these_users = jusers.find({})
    these_users = lusers.find({"iLikedPictures": False})
    #print(these_users[1])
    #print(these_users[2])
    #print(these_users[3])
    for user in these_users:
        ...
        if gender == "M":
            ...
            picture_links = [
                el.get_attribute("href") for el in picture_elements
            ]

            picture_links = [
                picture_links[x] for x in range(3) if len(picture_links) > 3
            ]
            ...
            update_data(lusers, "_id", user["_id"], "iLikedPictures", True,
                        False)
