...
def scrape_user_page(state):
    user_details_elements = driver.find_elements(By.XPATH,user_details_selector)
    user_details_text = [el.text for el in user_details_elements]
    view_picture_elements = driver.find_elements(By.XPATH, view_pictures_selector)
...
def get_user_pages():
    ...
    for state in states:
        ...
        for x in range(start_page, end_page):
            ...
            increment_data(locations, "name", state["name"],
                                   "scrapedPages", 1, False)
...
def love_pictures():
    ...
    for user in these_users:
        ...
        picture_links = [picture_links[x] for x in range(4) if len(picture_links) > 4]
